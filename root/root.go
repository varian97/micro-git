...
func InitDB() error {
	...
	masterRefFilePath := filepath.Join(refsHeadsFolderPath, "master")
	_, err = os.Create(masterRefFilePath)
	if err != nil {
		errorMessage := fmt.Errorf("failed to initialize master branch: %v", err)
		return errorMessage
	}
